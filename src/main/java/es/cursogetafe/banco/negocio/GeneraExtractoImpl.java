...
import es.cursogetafe.banco.excepciones.ClienteInexistenteException;
...
public class GeneraExtractoImpl implements GeneraExtracto {
	...
	public Cliente buscarUnCliente(int id) {
		Optional<Cliente> cliOp = cd.findById(id);
		if(cliOp.isPresent()) return cliOp.get();
		else throw new ClienteInexistenteException();
	}
